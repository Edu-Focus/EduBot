...
bot.on ('message', message => {
	...
	if(cmd === (prefix + 'help')) {
		const embed = new Discord.RichEmbed({
			...
			'footer': 'Légende : {} = Obligatoire || () = Optionnel',
		});
		...
		retreiveStaffList(key).then(function(list) {
			...
			Object(list).forEach(function(list) {
				...
				if(list.misc.discord_id != null) {
					const id_discord = `(<@${list.misc.discord_id}>)`;
				}
				else{
					const id_discord = '(Aucun compte discord relié)';
				}
	...
	if(cmd === (prefix + 'sysban')) {
		...
		if(args[1]) {
			const raison = message.content.split(' ').slice(2).join(' ');
		}
		else{
